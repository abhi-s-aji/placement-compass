...
export default function UserManagementClient({ initialUsers }: UserManagementClientProps) {
  ...
  async function handleChangeRole(userId: string, newRole: 'student' | 'mentor') {
    ...
    try {
      ...
      window.alert(`User role updated successfully to ${newRole}`);
    } catch (error) {
                    ...
                    <tr key={u.id} style={{ borderBottom: '1px solid var(--color-border-subtle)' }} className="table-row-hover">
                      ...
                      <td style={{ padding: 'var(--space-4)' }}>
                        <select
                          className="input"
                          value={u.role}
                          disabled={isUpdatingRole || u.role === 'admin'}
                          onChange={(e) => handleChangeRole(u.id, e.target.value as 'student' | 'mentor')}
